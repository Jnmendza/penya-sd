            ...
            <div key={app.id} className="px-6 py-4 flex flex-col md:flex-row md:items-center gap-4">
              ...
              <div className="flex gap-2 shrink-0">
                <button onClick={() => handleApproveClick(app.id)} disabled={loadingId === app.id}
                  className="rounded-lg bg-barca-blue px-4 py-1.5 text-xs font-bold text-white hover:bg-blue-900 shadow-sm disabled:opacity-50 cursor-pointer">
                  {loadingId === app.id ? "..." : "✓ Confirm Payment & Approve"}
                </button>
                <button onClick={() => handleRejectClick(app.id)} disabled={loadingId === app.id}
                  className="rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-600 hover:bg-slate-200 disabled:opacity-50 cursor-pointer">
                  ✕
                </button>
              </div>
            </div>
